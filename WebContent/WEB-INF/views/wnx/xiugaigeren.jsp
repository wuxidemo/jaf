	...
	<div class="weui_cells">
			...
			<div class="weui_cell" style="border-top: 1px solid #DCDDDD;">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<input class="weui_input" maxlength="5" id="myname" onblur="nam()"
						placeholder="请输入您的姓名" name="name" value="${volunteer.name}">
				</div>
			</div>
			<div class="weui_cell weui_cell_select weui_select_after" style="border-top: 1px solid #DCDDDD;padding-left: 30px;">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<select class="weui_select" id="mytype" name="sex">
						<option <c:if test="${volunteer.sex == 1 }">selected="selected"</c:if> value="1">男</option>
						<option <c:if test="${volunteer.sex == 2 }">selected="selected"</c:if> value="2">女</option>
					</select>
				</div>
			</div>
			<div class="weui_cell" style="border-top: 1px solid #DCDDDD;">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<input class="weui_input" maxlength="2" id="myage" onblur="myag()"
						placeholder="请输入您的年龄" name="age" value="${volunteer.age}" type="tel">
				</div>
			</div>
			<div class="weui_cell" style="border-top: 1px solid #DCDDDD;">
				<div class="weui_cell_hd">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<input class="weui_input" type="tel" maxlength="11" id="phone" onblur="myphone()"
						placeholder="请输入您的手机号码" name="phone" value="${volunteer.phone}">
				</div>
			</div>
			<div class="weui_cell" style="border-top: 1px solid #DCDDDD;display: none;" id="yzma">
				<div class="weui_cell_hd">
				...
				<div id="hqyzm" style="display: none;">
					...
					<table style="width:100%;">
						 <c:forEach items="${ablelist}" var="course" varStatus="cou">
						 	<c:if test="${cou.count%4 == 1 }"><tr style="height:50px;"></c:if>
						 		<td style="width:25%;">
						 			<div class="onecourse <c:if test="${fn:indexOf(ablestr,course.id) >= 0}">bgclass</c:if> " onclick="changecolor(this)" value="${course.id}">
										${course.value}
									</div>
						 		</td>
				...
				<div class="weui_cell">
					<div class="weui_cell_bd weui_cell_primary">
						<textarea id="edtt" class="weui_textarea" placeholder="请简单描述您的才能(不能超过50字)"
							rows="5" onblur="edit()" name="abilitydescrib" maxlength="50">${volunteer.abilitydescrib}</textarea>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="weui_cells" style="border-top: none;">
			<div class="weui_cell">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<input class="weui_input" id="fuwushijian" onblur="tim()" maxlength="13"
						placeholder="请填写您能提供服务的时间" name="servertime" value="${volunteer.servertime}">
				</div>
			</div>
			<div class="weui_cell weui_cell_select weui_select_after" style="border-top: 1px solid #DCDDDD;padding-left: 30px;">
				<div class="weui_cell_hd">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<select class="weui_select" id="paytype" name="paytype">
						<option <c:if test="${volunteer.paytype == 1}">selected=""</c:if>value="1">有偿</option>
						<option <c:if test="${volunteer.paytype == 0}">selected=""</c:if>value="0">无偿</option>
					</select>
				</div>
			</div>
			<div class="weui_cell" id="teshu" style="border-bottom: 1px solid #DCDDDD;border-top: 1px solid #DCDDDD;<c:if test="${volunteer.paytype == 0}">display:none;</c:if>">
				<div class="weui_cell_hd">
					<label class="weui_label">参&nbsp;考&nbsp;价</label>
				</div>
				<div class="weui_cell_bd weui_cell_primary">
					<input class="weui_input" id="cankaojia" onblur="price()"
						placeholder="请填写参考价" name="pay" maxlength="6" value="${volunteer.pay}">
				</div>
			</div>
			<input id="a" name="headimgurl" type="hidden" value="${volunteer.headimgurl}">
...
<script type="text/javascript">
	var oldname = '${wu.name}';
	var oldphone = '${wu.phone}';
	var oldsex = '${wu.sex}';
	var bianjiphone = '${volunteer.phone}';

	$(document).ready(function(){
		var denystate = '${volunteer.state}';
		if(denystate != 2) {
			$(".weui_allll").show();
			$("#onetishi").show();
			$("#onetishi2").show();
			document.getElementById("weui_dialog_bd").innerHTML = "修改才能类型和才能描述后会重新审核,请谨慎修改!";
		}
		getcommunitydata();
		wx.config({
			debug : false,
...
//提交事件	
	...
	$("#phone").bind("keyup blur keydown", function(){
		if($("#phone").val() == bianjiphone){
			$("#hqyzm").hide();
			$("#yzma").hide();
		}else if($("#phone").val() != bianjiphone){
			$("#hqyzm").show();
			$("#yzma").show();
		}
	});
	...
	function returnhome(){
		window.location.href = "${ctx}/wxcommunity/?comid=${comid}";
	}
