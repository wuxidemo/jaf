...
<body>
		...
		<div class="span12">
			<h3 class="page-title">
				<img src="${ctx}/static/images/shgl.png"
					style="vertical-align: text-bottom;" />关注分享抽奖
			</h3>
		</div>
	</div>
	...
	<div class="portlet box grey">
		<div class="portlet-title">
			...
			<div class="actions">
				<a href="javascript:void(0);" onclick="down()" target="_blank"
					class="btn blue">下载</a> <a href="javascript:void(0);"
					onclick="down1()" target="_blank" class="btn blue">下载手机号</a>
			</div>
		</div>
...
<script type="text/javascript">
	...
	jQuery(document).ready(
			function() {
				...
				$('#time').bind(
						...
						function() {
							if ($(this).val() != "") {
								$.post("${ctx}/sharerecord/getwindata?time="
										+ $(this).val() + "&type=gzfxhd",
										function(d) {
											setalldata(d);
										});
							}
	...
	function setdata(count, kjid) {
		...
		if (time != null) {
			$
					.post(
							"${ctx}/sharerecord/luckdata?time=" + time
									+ "&type=gzfxhd&count=" + count + "&ids="
									+ getnowdata(kjid),
							function(d) {
								if (d.result == 2) {
	...
	function saverr(kjid) {
		...
		$.post("${ctx}/sharerecord/savewin?ids=" + ids + "&sort=" + kjid
	...
	function down() {
		window.open("${ctx}/ticketrecord/fxoutdata?time=" + gettime());
	}

	function down1() {
		window.open("${ctx}/ticketrecord/fxoutdata1?time=" + gettime());
	}
</script>
</html>