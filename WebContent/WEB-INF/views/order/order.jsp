...
<script type="text/javascript">
	...
	function showdetail(id) {
		$
				.post(
						"${ctx}/order/detail?id=" + id,
						function(d) {
							var html = '<table style="width: 100%;"><thead><tr><th style="width: 80px"></th><th style="width: %"></th><th style="width: 80px"></th><th style="width: %"></th><th style="width: 80px"></th><th style="width: %"></th></tr></thead><tr><td>订单号:</td><td>'
									+ d.order.code
									+ '</td><td>交易时间：</td><td>'
									+ getLocalTime(new Date(d.order.paytime))
									+ '</td><td>微信号:</td><td>'
									+ (d.order.payname == null ? ''
											: d.order.payname)
									+ '</td>	</tr><tr><td>交易金额:</td><td>'
									+ d.order.price/100.0
									+ '</td><td>支付金额：</td>	<td>'
									+ d.order.payprice/100.0
									+ '</td><td>状态:</td><td>'
									+ (d.order.state == 3 ? '已结算' : '未结算')
									+ '</td>	</tr></table>';
							for (var i = 0; i < d.cards.length; i++) {
								var type = "";
								if (d.cards[i].cardtype == "CASH") {
									type = "代金券";
								} else if (d.cards[i].cardtype == "DISCOUNT") {
									type = "折扣券";
								} else {
									type = "礼品券";
								}
								html += '<div	style="border: 1px dashed; margin-top: 10px; padding: 10px 0; width: 100%;"><table style="width: 90%;"><thead><tr><th style="width: 80px"></th><th style="width: %"></th><th style="width: 80px"></th><th style="width: %"></th><th style="width: 80px"></th><th style="width: %"></th></tr></thead><tr><td>卡券类型:</td><td>'
										+ type
										+ '</td><td>卡券名称：</td>	<td>'
										+ d.cards[i].cardname
										+ '</td><td></td><td></td></tr><tr><td>卡券额度:</td>	<td>'
										+ (d.cards[i].bankprice + d.cards[i].shopprice)
										/ 100.0
										+ '</td><td>农商行:</td><td>'
										+ (d.cards[i].bankprice)
										/ 100.0
...
<body>
	<div class="modal hide fade" id="detail" tabindex="-1" role="dialog"
		...
		<div id="ordercontent" class="modal-body"></div>
	...
	<div class="portlet box grey" style="margin-bottom: 0px; height: auto;">
		...
		<div class="portlet-body" style="padding-top: 25px;">
			<form class="form-search" id="myform" action="${ctx}/order " method="post">
	...
	<div class="portlet box grey">
		...
		<div class="portlet-body">
			...
			<div class="row-fluid"
				...
				&nbsp;&nbsp;&nbsp;卡券（张）：${cardcount}&nbsp;&nbsp;&nbsp;卡券总额（元）：${cardbankprice}</div>
