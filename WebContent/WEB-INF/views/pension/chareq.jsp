	...
	<div id="change_htm">
		<form action="" id="inputForm">
			<div class="weui_cells">
				...
				<div class="weui_cell weui_cells_form">
					...
					<div class="weui_cell_bd weui_cell_primary">
						<input class="weui_input" maxlength="5" name="name"
							value="${penapply.name}" onblur="checkClear(this,0,'请输入姓名')"
						...
						<c:choose>
							<c:when test="${penapply.sex==1}">
								<input class="weui_input radio_yw" type="radio" name="sex"
									id="boy" value="1" checked="checked" />
								<label for="boy" onclick="choBoy()">男</label>
								<input class="weui_input" type="radio" name="sex" id="gril"
									value="2" />
								<label for="gril" onclick="choGril()">女</label>
							</c:when>
							<c:otherwise>
								<input class="weui_input" type="radio" name="sex" id="boy"
									value="1" />
								<label for="boy" onclick="choBoy()">男</label>
								<input class="weui_input radio_yw" type="radio" name="sex"
									id="gril" value="2" checked="checked" />
								<label for="gril" onclick="choGril()">女</label>
							</c:otherwise>
						</c:choose>
					</div>
				...
				<div class="weui_cell weui_cells_form">
					...
					<div class="weui_cell_bd weui_cell_primary">
						<input class="weui_input" maxlength="2" name="age"
							value="${penapply.age}" onblur="checkClear(this,0,'请输入姓名')"
				...
				<div class="weui_cell weui_cells_form">
					...
					<div class="weui_cell_bd weui_cell_primary">
						<input class="weui_input" type="tel" maxlength="11" id="phone"
							...
							name="telephone" value="${penapply.telephone}">
					</div>
				...
				<div class="weui_cell weui_cells_form" style="border-bottom: 1px solid #eeeff0;">
					<div class="weui_cell_hd">
						<label class="weui_label">家庭住址</label>
					</div>
					<div class="weui_cell_bd weui_cell_primary">
						<input class="weui_input" type="" maxlength="30" id="address_home"
							onblur="checkClear(this,0,'')" onkeyup="onchangeClear(this)" name="address"
							placeholder="请填写您的家庭住址" value="${penapply.address}" />
					</div>
					<div class="weui_cell_ft">
						<i class="weui_icon_warn"></i>
					</div>
					...
					<div class="weui_cell weui_cells_form"
						...
						<div class="weui_cell_bd weui_cell_primary">
							<textarea class="weui_textarea" name="content" maxlength="200"
								...
								placeholder="请简单描述您的要求（包括服务时间、服务需求）..." rows="4">${penapply.content}</textarea>
						</div>
					</div>
				</c:if>
			</div>
			<!-- 后台传递数据 -->
			<input type="hidden" name='type' value="${penapply.type}">
			<c:if test="${penapply.pensionAct.id != null && penapply.pensionAct.id != ''}">
				<input type="hidden" name='pensionAct.id' value="${penapply.pensionAct.id}">
			</c:if>
			<input type="hidden" name="openid" value="${openid}">
			<input type="hidden" name="id" value="${penapply.id}">
		</form>
		<!-- 按钮 -->
			...
			<div class="button">
				<div class="bd spacing">
					<a href="javascript:;" onclick="yzComparison()"
						class="weui_btn weui_btn_primary">保存修改</a>
				</div>
			</div>
		</div>
	</div>
...
function yzComparison() {
	...
	if ("" == name.trim() || null == name) {
		...
		showHint("请填写家庭住址");
	}else if(!'${penapply.pensionAct.id}'){
...
//跳转页面
function gotopelmeg(){
	window.location.href = "${ctx}/wxcommunity/pensiondetail?id=${penapply.pensionAct.id}";
}
//跳转页面
function gotopelact(){
