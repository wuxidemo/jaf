...
<script>
 ...
 /* 查询当天的数据 */
 function event222(){
		$.post("${ctx}/wxpage/getactivity",function(d){
			$("#bdy").empty();
			   if(d.result == "1"){
				   var list=d.listactivityday;
 ...
 /*   查询最近七天的数据 */
  function event1(){	
   $.post("${ctx}/wxpage/getsevenactivity",function(d){
			
		$("#bdy").empty();
		   if(d.result == "1"){
			   var list=d.listactivityseven;
			   for(var i=0;i<list.length;i++){
				   $("#bdy").append('<tr>'
						             +'<td>'+(i+1)
						             +'</td>'
 ...
 /* 查询最近30天的数据 */
  function event2(){
    $.post("${ctx}/wxpage/getthirtyactivity",function(d){
			
    	$("#bdy").empty();
		   if(d.result == "1"){
			   var list=d.listactivitythirty;
			   for(var i=0;i<list.length;i++){
				   $("#bdy").append(' <tr>'
						             +'<td>'+(i+1)
						             +'</td><td>'+list[i][1]
     ...
     /*   查询所有的点击记录 */
     function event3(){
    	 $.post("${ctx}/wxpage/getactivitysum",function(d){
    	
    	    	$("#bdy").empty();
    			   if(d.result == "1"){
    				   var list=d.listactivitysum;
    				   for(var i=0;i<list.length;i++){
 ...
 function but(){
	 ...
	 if($.trim(actname).length == 0 && $.trim(start).length == 0) {
	 ...
	 $.post("${ctx}/wxpage/getactivityzhuhe",{"start":start,"end":end,'name':actname},function(d){
...
<body>
	<div class="row-fluid">
		<div class="span12">
			<h3 class="page-title">
				<img src="${ctx}/static/images/xtgl.png">活动统计
			</h3>
		</div>
	</div>
			...
			<table style="width: 100%">
				...
				<tr>
					<td colspan="3" style="width: 1%"><label class="control-label"
						style="float: left; width: 80px; padding-left: 4%; margin-top: 7px">活动名称：</label>
