...
<%
	...
	if (request.getParameter("search_EQ_categoryValue.id") != null)
		district = new String(request.getParameter(
				"search_EQ_categoryValue.id").getBytes("ISO-8859-1"),
				"UTF-8");
%>
...
$(function(){
	...
	function sureDel() {
		...
		$.post('${ctx}/community/delete', {
			...
			if (data.result) {
				window.location.href = '${ctx}/community';
...
<body>
	<div class="row-fluid">
		<div class="span12">
			<h3 class="page-title">
				<img src="${ctx}/static/images/xtgl.png" style="vertical-align: text-bottom;" /> 社区管理
			</h3>
		</div>
	</div>
	...
	<div class="portlet box grey" style="margin-bottom: 0px; height: auto;">
		...
		<div class="portlet-body" style="padding-top: 25px;">
			<form class="form-search" action="${ctx}/community" method="post">
				<table style="width: 100%">
					<tr>
						<td style="width: 33%;"><label class="control-label" style="float: left">社区名称：</label> <input type="text" id="search_name" name="search_LIKE_name" maxlength="20" style="float: left; width: 60%; height: 32px;" class="" value="${LIKE_name }"></td>
						<td style="width: 33%;"><label class="control-label" style="float: left">社区区域：</label> <select id="search_district" name="search_EQ_categoryValue.id" maxlength="20" style="float: left; width: 60%; height: 32px;" class="">
	...
	<div class="portlet box grey">
		<div class="portlet-title">
			...
			<div class="actions">
				<a href="javascript:;" class="btn blue" onclick="show_add()"><i class=""></i> 新增</a> <a href="javascript:;" class="btn red" onclick="deleteCommunity()"><i class=""></i>删除</a>
			</div>
		</div>
		<div class="portlet-body">
			...
			<table id="contentTable" class="table table-striped table-bordered table-hover">
				...
				<tbody>
					<c:if test="${community.totalPages!=0}">
						<c:forEach items="${community.content}" var="community" varStatus="status">
							<tr>
								<td><input type="checkbox" id="check_item" value="${community.id}" class="check_item"></td>
								<td>${status.count}</td>
								<td>${community.name}</td>
								<td>${community.categoryValue.value}</td>
								<td>${community.user.realname}</td>
								<td>${fn:substring(community.createtime, 0, 16)}</td>
								<td>
									<a href="javascript:;" onclick="show_edit('${community.id}','${community.name}','${community.categoryValue.id}','${community.telephone}')">修改</a>
									<a href="javascript:;" onclick="showaddadmin('${community.id}')">添加管理员</a>
									<a href="javascript:;" onclick="showadminlist('${community.id}')">查看管理员列表</a>
								</td>
							</tr>
						</c:forEach>
					</c:if>
			...
			<tags:pagination page="${community}" paginationSize="5" />
		</div>
	</div>
	<!-- 模态窗口 -->
	<div class="modal hide fade" id="form_community" tabindex="-1" role="dialog" style="top: 10%; width: 50%; left: 40%;">
		<div class="modal-header">
			<button class="close" type="button" data-dismiss="modal"></button>
			<h3 id="myModalLabel">
				<span id="form_title"> 新增/修改社区
				</span>
			</h3>
		</div>
		<div id="" class="modal-body" style="font-size: 35;">
			<form action="${ctx}/community/create" method="post" id="inputForm" class="form-horizontal">
				<div class="control-group">
					<label class="control-label">社区名称:</label>
					<div class="controls">
						<input type="text" id="name" name="name" maxlength="20" style="width: 60%; height: 32px" class="title required" />
					</div>
				</div>
				<div class="control-group">
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="text" id="telephone" name="telephone" maxlength="12" style="width: 60%; height: 32px" class="title required" /> <input type="hidden" id="id" name="id" value="">
					</div>
				</div>
			</form>
		</div>
	...
	<div class="modal hide fade" id="openaccount" tabindex="-2" role="dialog" style="top: 10%;width:800px;left:40%;">
		...
		<div id="" class="modal-body">
			<form action="${ctx}/community/addadmin" method="post" id="opencountform" class="form-horizontal">
		...
		<div class="modal-footer">
			...
			<input class="btn btn-primary grey" type="button" onclick="hideopen()" value="取消" />
		</div>
	</div>
	
	<div class="modal hide fade" id="showadminlistdiv" tabindex="-2" role="dialog" style="top: 10%;width:800px;left:40%;">
		<div class="modal-header">
			<button class="close" type="button" data-dismiss="modal"></button>
		...
		<div class="modal-footer">
			<input class="btn btn-primary green" type="button" onclick="adminlisthide()" value="确定" />&nbsp; 
		</div>
	</div>
	...
	<script type="text/javascript">
			...
			if(phoneno.trim() == '') {
				...
				$.get(checkurl,function(data){
					if(data == 'false') {
						...
						$.post(sendurl,{"communityid":communityid,"phoneno":phoneno,"uname":uname},function(data){
							hideopen();
							window.parent.showAlert("创建社区管理员成功，登陆用户名为"+phoneno+"，初始登陆密码为  000000 ");
						});
					}
				});
			}
		...
		function showadminlist(commid) {
			...
			$.post(posturl,{"commid":commid},function(data){
				if(data.result == '1') {
					var d = data.data;
					for(var i=0; i<d.length; i++) {
						$("#adminlist").append('<tr class="odd gradeX">'
								 + '<td>'
								 + (i+1)
								 + '</td>'
								 + '<td>'
								 ...
								 + '</td>'
								 + '<td>'
								 + (d[i].enabled == '0' ? "<span class=\"label label-warning\">冻结</span>" : "<span class=\"label label-success\">正常</span>")
								 + '</td>'
								 + '<td>'
								 + (d[i].enabled == '0' ? "<a href=\"javascript:;\" onclick=\"askforenable("+d[i].id+")\">解冻账号</a>&nbsp;" : "<a href=\"javascript:;\" onclick=\"askfordisable("+d[i].id+")\">冻结账号</a>&nbsp;")
								 + '<a href="javascript:;" onclick="askforresetpass('+d[i].id+')">重置密码</a>'
								 + '</td>'
								 +'</tr>');
					}
				}else{
		...
		function suredisable() {
			...
			$.post(disurl,{"uid":tempuserid},function(data){
				//adminlisthide();
		...
		function sureenable() {
			...
			$.post(ableurl,{"uid":tempuserid},function(data){
				//adminlisthide();
				showadminlist(shequid);
				window.parent.showAlert(data.msg);
			});
		...
		function resetpass() {
			...
			$.post(reseturl,{"uid":tempuserid},function(data){
				//adminlisthide();
				showadminlist(shequid);
				window.parent.showAlert(data.msg);
			});
