...
(function(b) {
	var a = {
		init : function(c) {
			return this.each(function() {
				...
				this.opt.number = a._between(this.opt.number, 0,
						this.opt.numberMax);
				this.opt.path = this.opt.path || "";
				if (this.opt.path
						&& this.opt.path.slice(this.opt.path.length - 1,
				...
				var f = this.opt.space ? 4 : 0, d = this.opt.width
						|| (this.opt.number * this.opt.size + this.opt.number
								* f);
				if (this.opt.cancel) {
		...
		_bindClick : function() {
			...
			c.stars.on("click.raty", function(e) {
				c.score.val((c.opt.half || c.opt.precision) ? d.data("score")
						: this.alt);
				if (c.opt.click) {
		...
		_createCancel : function() {
			var e = b(this), c = this.opt.path + this.opt.cancelOff, d = b(
					"<img />", {
						src : c,
						alt : "x",
		...
		_createStars : function() {
			...
			for (var c = 1; c <= this.opt.number; c++) {
				var f = a._getHint.call(this, c), d = (this.opt.score && this.opt.score >= c) ? "starOn"
						: "starOff";
		...
		_fill : function(d) {
			...
			for (var f = 1; f <= m.stars.length; f++) {
				var g = m.stars.eq(f - 1), l = m.opt.single ? (f == d)
						: (f <= d);
				if (m.opt.iconRange && m.opt.iconRange.length > e) {
					var j = m.opt.iconRange[e], h = j.on || m.opt.starOn, c = j.off
							|| m.opt.starOff, k = l ? h : c;
					if (f <= j.range) {
		...
		_lock : function() {
			var d = parseInt(this.score.val(), 10), c = d ? a._getHint.call(
					this, d) : this.opt.noRatedMsg;
		...
		_roundStars : function(e) {
			...
			if (d > this.opt.round.down) {
				...
				this.stars.eq(Math.ceil(e) - 1).attr("src",
						this.opt.path + this.opt[c]);
			}
		},
	...
	b.fn.raty.defaults = {
		...
		half : false,
		halfShow : true,
		hints : [ "非常失望", "差强人意", "感觉不错", "物超所值", "强烈推荐" ],
		iconRange : undefined,
		mouseout : undefined,
		mouseover : undefined,
