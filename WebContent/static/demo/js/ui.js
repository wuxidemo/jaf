...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        progressBarWrapper.addClass("progress progress-striped active");
...
FileProgress.prototype.reset = function() {
    ...
    this.fileProgressWrapper.find('td .progress .bar').attr('aria-valuenow', 0).width('0%').find('span').text('');
    this.appear();
};
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    var progressbar = this.fileProgressWrapper.find('td .progress').find('.bar');
...
FileProgress.prototype.setComplete = function(up, info,isimg) {
 ...
 //   }
    ...
    td.html("<a onclick='deletetr(\""+td.parent().parent().parent().attr("id")+"\")' >删除</a>").removeClass().next().next('.status').hide();
	td.parent().parent().parent().attr("url",up.getOption('domain')+ encodeURI(res.key));
...
FileProgress.prototype.setCancelled = function(manual) {
    ...
    this.fileProgressWrapper.attr('class', progressContainer);
    this.fileProgressWrapper.find('td .progress .bar').css('width', 0);
};
